...
void
LinuxSerialDevice::lockMemory()
{
  pthread_mutex_lock(&m_memLock);
}

void
LinuxSerialDevice::freeMemory()
{
  pthread_mutex_unlock(&m_memLock);
}

void
LinuxSerialDevice::lockMSG()
{
  pthread_mutex_lock(&m_msgLock);
}

void
LinuxSerialDevice::freeMSG()
{
  pthread_mutex_unlock(&m_msgLock);
}

void
LinuxSerialDevice::lockACK()
{
  pthread_mutex_lock(&m_ackLock);
}

void
LinuxSerialDevice::freeACK()
{
  pthread_mutex_unlock(&m_ackLock);
}

void
LinuxSerialDevice::notify()
{
  pthread_cond_signal(&m_ackRecvCv);
}

void
LinuxSerialDevice::lockProtocolHeader()
{
  pthread_mutex_lock(&m_headerLock);
}

void
LinuxSerialDevice::freeProtocolHeader()
{
  pthread_mutex_unlock(&m_headerLock);
}

void
LinuxSerialDevice::lockNonBlockCBAck()
{
  pthread_mutex_lock(&m_nbAckLock);
}

void
LinuxSerialDevice::freeNonBlockCBAck()
{
  pthread_mutex_unlock(&m_nbAckLock);
}

void
LinuxSerialDevice::notifyNonBlockCBAckRecv()
{
  pthread_cond_signal(&m_nbAckRecv);
}

void
LinuxSerialDevice::nonBlockWait()
{
...
void
LinuxSerialDevice::wait(int timeoutInSeconds)
{
...
****/
void
LinuxSerialDevice::setBaudrate(unsigned int baudrate)
{
  m_baudrate = baudrate;
}

void
LinuxSerialDevice::setDevice(std::string device)
{
  m_device = device;
}
...
int
LinuxSerialDevice::_serialWrite(const unsigned char* buf, int len)
{
...
int
LinuxSerialDevice::_serialRead(unsigned char* buf, int len)
{
  int ret = -1;
